module Admin
  class Admin::JobsController < Admin::AdminController
    ...
    def block
      job.active ^= true
      job.published = true if job.active
      job.published_at = DateTime.now() if job.published_at.blank?
      job.save
    ...
    private
      def job_params
        params.require(:job).permit(:name, :email, :detail, :photo, :url, :city, :state, :country, :published, :views, 
